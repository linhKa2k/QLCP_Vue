      ...
      if (Array.isArray(val.data.content) && val.data.content.length > 0) {
        ...
        if (
          ...
          res.document_file_list.forEach((file: any) => {
            if (file.position == pdfSignPosition.PHU_LUC_CHI_PHI) {
              dataFileAppendix.value.push(file);
            } else if (
              file.position == pdfSignPosition.DE_NGHI_TAM_UNG ||
              file.position == pdfSignPosition.KE_HOACH_CONG_TAC ||
              file.position ==
