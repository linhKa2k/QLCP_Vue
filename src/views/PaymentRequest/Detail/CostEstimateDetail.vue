<template>
  <a-col :span="24">
    <a-row type="flex" :gutter="[16, 0]">
      <a-col :span="6">
        <a-tabs v-model:activeKey="activeKey" style="margin: 5%">
...
queryClient.invalidateQueries(ServerStateKeysEnum.plan_cost);
queryClient.invalidateQueries(ServerStateKeysEnum.plan_route);
queryClient.invalidateQueries(ServerStateKeysEnum.plan_route_user);
const columns = [
  {
    title: "Nội dung",
    dataIndex: "content",
...
const onSearchStaff = () => {
  dataStaffItem.value = dataStaffRecord.value.filter((item) => {
    return (
...
const { isLoading: staffLoading } = getPlanRouteUser(planParams, {
  ...
  onSuccess: async (val) => {
    ...
    dataStaffItem.value = await dataStaffRecord.value;
  },
});
