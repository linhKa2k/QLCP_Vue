...
export default defineComponent({
  ...
  methods: {
      ...
      exportSample(dispatch, payload) {
        dispatch(camelCase(this.entity) + "/exportSample", payload).then(
          (response) => {
            ...
            let fileName = `Danh_sach_mau_de_nghi_chuyen_khoan.xlsx`;
            FileSaver.saveAs(new Blob([response.data]), fileName);
          }
        );
      },
    ...
    updateStatusTransferRequest() {
      TransferRequestService.updateStatus({
        id: this.record.id,
        status: constant.transferRequestStatus.DA_CAP_NHAT,
      })
        .then((response) => {
    ...
    updateSummaryRequest() {
      TransferRequestService.detail(this.record.id)
        .then((response) => {
          if (response.status == 200) {
            let transfer_request_items = response.data?.transfer_request_item;
            if (transfer_request_items.length > 0) {
              transfer_request_items.map((p) => {
                if (p.type == constant.summaryRequestType.DE_NGHI_CHI) {
                  PaymentRequestService.updateStatus({
                    id: p.entity_id,
                    status: p.status,
                    reason: "",
                  })
                ...
                if (p.type == constant.summaryRequestType.DE_NGHI_TAM_UNG) {
                  PlanRequiredService.updateStatus({
                    id: p.entity_id,
                    status: p.status,
                    reason: "",
                  })
      ...
      this.import(payload)
        .then((response) => {
          if (response.status == 200) {
            ...
            this.updateStatusTransferRequest();
          } else {
            this.confirmLoading = false;
            message.error(response.statusText);
          }
