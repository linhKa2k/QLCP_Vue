  ...
  <a-row>
    <a-col :span="24">
      <a-card
        :bordered="false"
        class="card-border"
        :title="$t('livingQuota.title.general_information')"
      >
      ...
      <a-card
        :bordered="false"
        class="card-border"
        :title="$t('livingQuota.title.additional_information')"
      >
        <a-row type="flex" justify="center" align="middle">
          <a-col :span="24">
            <a-form layout="vertical" :model="form">
...
import { formatDate, formatIsActive, formatCurrency } from "@/utility";
...
const renderType = (type) => {
  const item = Constant.livingQuotaType.find((i) => i.value == type);
  return item?.label;
};
